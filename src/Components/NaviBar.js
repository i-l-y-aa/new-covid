      ...
      <Navbar.Toggle aria-controls="responsive-navbar-nav" />
        ...
        <Nav.Link><NavLink to="/news" className={setActive} ><h5>Новости</h5></NavLink></Nav.Link>
          ...
          <h5><NavDropdown title={<span className="fdrop">Статистика</span>} id="collasible-nav-dropdown">
            ...
            <NavDropdown.Item> <NavLink to="/tub" className={({isActive}) => isActive ? 'active-dropdown' : 'non-active-dropdown'} >Туберкулёз (эпидемиологические данные)</NavLink></NavDropdown.Item>
            <NavDropdown.Item> <NavLink to="/tub_soc" className={({isActive}) => isActive ? 'active-dropdown' : 'non-active-dropdown'} >Туберкулёз (социально-экономические данные)</NavLink></NavDropdown.Item>
          </NavDropdown></h5>
