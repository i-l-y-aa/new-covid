...
export function Life_b_En () {
  const [views9, setViews9] = useState(0);

  useEffect(() => {
    const storedViews9 = localStorage.getItem('sem_e_views9');
    const newViews9 = storedViews9 ? parseInt(storedViews9) + 1 : 1;
    setViews9(newViews9);
    localStorage.setItem('sem_e_views9', newViews9);
  }, []);
  return (
        <>
    <NaviBarv2_En />
    ...
    <div className="row ps-4 my-4">
          <p className="lead">
            <FontAwesomeIcon icon={faEye} /> {views9}
          </p>
        </div>
    </Container>
    <Footer_En />
