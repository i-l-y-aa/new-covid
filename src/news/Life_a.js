...
export function Life_a () {
  const [views8, setViews8] = useState(0);

  useEffect(() => {
    const storedViews8 = localStorage.getItem('sem_e_views8');
    const newViews8 = storedViews8 ? parseInt(storedViews8) + 1 : 1;
    setViews8(newViews8);
    localStorage.setItem('sem_e_views8', newViews8);
  }, []);
  return (
        <>
    <NaviBarv2 />
    ...
    <div className="row ps-4 my-4">
          <p className="lead">
            <FontAwesomeIcon icon={faEye} /> {views8}
          </p>
        </div>
    </Container>
    <Footer />
